...
double x3f_calc_spatial_gain(x3f_spatial_gain_corr_t *corr, int corr_num,
    ...
    for (i = 0; i < corr_num; i++) {
        ...
        int co1, co2;
        double gr1, gr2;
        int channel = chan - c->chan;

        if (channel < 0 || channel >= c->channels) continue;

        if (row % c->rowpitch != c->rowoff) continue;
        if (col % c->colpitch != c->coloff) continue;

        rc = rrel * (c->rows - 1);
        ri = (int) floor(rc);  //  largest integer value less than or equal to rc.
        rf = rc - ri;
        ...
        if (ci >= c->cols - 1)
            ...
            co1 = ci * c->channels + channel;
            co2 = (ci + 1) * c->channels + channel;
        }
