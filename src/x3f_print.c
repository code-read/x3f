...
// I haven't seen an output value of over 5 digits from this,
// so reduced %12 to %8 (below) for legibility - crw
static void print_matrix_element(FILE *f_out, camf_entry_t *entry, uint32_t i) {
    switch (entry->matrix_decoded_type) {
        case M_FLOAT:
            fprintf(f_out, "%8g ", ((double *)   (entry->matrix_decoded))[i]);
            break;
        case M_INT:
            fprintf(f_out, "%8d ", ((int32_t *)  (entry->matrix_decoded))[i]);
            break;
        case M_UINT:
            fprintf(f_out, "%8d ", ((uint32_t *) (entry->matrix_decoded))[i]);
            break;
    }
}
