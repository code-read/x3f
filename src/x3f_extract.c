...
// workaround because Standard C++ and MinGW do not include strnlen() - crw:
static __inline__
...
static void usage(char *progname) {
    ...
    "Usage: %s <SWITCHES> <file1> ...\n"
                    ...
                    "                   'none' means neither scale, apply gamma\n"
                    "                     nor convert color space.\n"
...
int main(int argc, char *argv[]) {
    ...
    // todo: use Standard C Library getopt() to make the following more legible/manageable :
    ...
    // Iterate over each file specified on command line:
    for (; i < argc; i++) {
        char *infile = argv[i];
